...
class EpisodesMatcher():
    ...
    def match(self, result_item):
        parsed_url = urllib.unquote(result_item.result.value).encode('utf-8').lower()
        if (parsed_url.find(result_item.result.item.episodeitem.series.lower().replace(' ', '')) >= 0 and
